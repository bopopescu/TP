...
from apps.multisensor.models import Multisensor
...
def multisensor(request, mac):
    print 'inside multisensor view method'
    ...
    print "unside multisensor view for mac: {}".format(mac)
...
def submit_changes(request):
    print 'inside multisensor update device method'
    if request.POST:
        _data = request.body
        print _data
        ...
        multisensor_update_send_topic = '/ui/agent/'+device_info[1]+'/update/bemoss/'+device_info[0]+'/'+device_info[2]
        print multisensor_update_send_topic
        zmq_pub.sendToAgent(multisensor_update_send_topic, _data, content_type, fromUI)
        print "success in sending message to agent"

    if request.is_ajax():
            return HttpResponse(json.dumps(_data), mimetype='application/json')

def get_multisensor_current_status(request):
    print "getting current status of multisensor"
    if request.method == 'POST':
        ...
        info_required = "Update multisensor data"

        #device_info = device_info.split('/')  # e.g. 999/lighting/1NST18b43017e76a
        # TODO fix building name -> should be changeable from 'bemoss'
        multisensor_update_send_topic = '/ui/agent/'+device_info[1]+'/device_status/bemoss/'+device_info[0]+'/'+device_info[2]
        print multisensor_update_send_topic

        zmq_pub.requestAgent(multisensor_update_send_topic, info_required, "text/plain", "UI")
