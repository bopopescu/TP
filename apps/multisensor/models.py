...
#multisensor device information - for all multisensor controllers in BEMOSS
class Multisensor(models.Model):
    multisensor = models.ForeignKey(DeviceMetadata, primary_key=True, max_length=50)
    ...
    class Meta:
        db_table = "multisensor"

    def __unicode__(self):
        return self.multisensor_id
    ...
    def data_as_json(self):
        zone_req = Building_Zone.as_json(self.zone)
        device_info = DeviceMetadata.objects.get(device_id=self.multisensor_id)
        metadata = DeviceMetadata.data_as_json(device_info)
        return dict(
            id=self.multisensor_id,
            status=self.status,
            power=self.power,
            energy=self.energy,
            zone=zone_req,
