...
def inverter(request, mac):
    print 'inside tv view method'
...
def submit_changes(request):
    ...
    if request.POST:
        ...
        tv_update_send_topic = '/ui/agent/'+device_info[1]+'/update/bemoss/'+device_info[0]+'/'+device_info[2]
        print tv_update_send_topic
        zmq_pub.sendToAgent(tv_update_send_topic, _data, content_type, fromUI)
        print "success in sending message to agent"

    if request.is_ajax():
            return HttpResponse(json.dumps(_data), mimetype='application/json')

def get_tv_current_status(request):
    print "getting current status of tv"
    if request.method == 'POST':
        ...
        info_required = "Update tv data"

        #device_info = device_info.split('/')  # e.g. 999/lighting/1NST18b43017e76a
        # TODO fix building name -> should be changeable from 'bemoss'
        tv_update_send_topic = '/ui/agent/'+device_info[1]+'/device_status/bemoss/'+device_info[0]+'/'+device_info[2]
        print tv_update_send_topic

        zmq_pub.requestAgent(tv_update_send_topic, info_required, "text/plain", "UI")
