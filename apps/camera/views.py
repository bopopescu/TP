...
def camera(request, mac):
    print 'inside camera view method'
    ...
    print "inside camera view for mac: {}".format(mac)
    ...
    return render_to_response(
        ...
        # context)
        {'device_type': "camera", 'mac_address': mac}, context)

@login_required(login_url='/login/')
def submit_changes(request):
    print 'inside camera update device method'
    if request.POST:
        _data = request.body
        print _data
        ...
        camera_update_send_topic = '/ui/agent/'+device_info[1]+'/update/bemoss/'+device_info[0]+'/'+device_info[2]
        print camera_update_send_topic
        zmq_pub.sendToAgent(camera_update_send_topic, _data, content_type, fromUI)
        print "success in sending message to agent"

    if request.is_ajax():
            return HttpResponse(json.dumps(_data), mimetype='application/json')

def get_camera_current_status(request):
    print "getting current status of camera"
    if request.method == 'POST':
        ...
        info_required = "Update camera data"

        #device_info = device_info.split('/')  # e.g. 999/lighting/1NST18b43017e76a
        # TODO fix building name -> should be changeable from 'bemoss'
        camera_update_send_topic = '/ui/agent/'+device_info[1]+'/device_status/bemoss/'+device_info[0]+'/'+device_info[2]
        print camera_update_send_topic

        zmq_pub.requestAgent(camera_update_send_topic, info_required, "text/plain", "UI")
...
@login_required(login_url='/login/')
def update_device_camera(request):
    print 'inside camera update device method'
    if request.POST:
        _data = request.body
        _data = json.loads(_data)
        ...
        content_type = "application/json"
        fromUI = "UI"
        lighting_update_send_topic = '/ui/agent/camera/update/bemoss/999/'+device_id
